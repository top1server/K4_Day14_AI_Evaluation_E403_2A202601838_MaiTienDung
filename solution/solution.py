...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score the response against each "
            "criterion from 0.0 to 1.0. Return a JSON object mapping criterion "
            "names to numeric scores.\n\n"
            f"Question:\n{question}\n\n"
            f"Response:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        score_data = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
        scores: dict[str, float] = {}
        for criterion in rubric:
            try:
                score = float(score_data[criterion])
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        scores_by_position: dict[str, list[float]] = {"first": [], "second": []}
        ...
        for result in scores_batch:
            ...
            position = str(result.get("position", "")).lower()
            if position in scores_by_position:
                scores_by_position[position].extend(numeric_scores)

        average = sum(all_scores) / len(all_scores) if all_scores else 0.0
        positional_bias = False
        if scores_by_position["first"] and scores_by_position["second"]:
            first_average = sum(scores_by_position["first"]) / len(scores_by_position["first"])
            second_average = sum(scores_by_position["second"]) / len(scores_by_position["second"])
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # The evaluator creates a convenient QAPair for standalone use;
            # the benchmark must retain the original pair and its metadata.
            result.qa_pair = pair
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review failure and add a targeted regression test"

            def clean(value: object) -> str:
                return str(value).replace("|", "\\|").replace("\n", " ")

            lines.append(
                f"| F{index:03d} | {clean(failure_type)} | {clean(root_cause)} | "
                f"{clean(suggestion)} | Open |"
            )
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": "Add an evidence/faithfulness guardrail to filter unsupported claims",
            "irrelevant": "Clarify the answer prompt and strengthen intent-aware retrieval",
            "incomplete": "Increase context coverage and add few-shot examples of complete answers",
            "off_topic": "Improve intent detection and add off-topic regression cases",
            "refusal": "Review refusal guardrails and add safe in-scope examples",
        }
        for failure_type in categories:
            suggestion = suggestion_by_type.get(failure_type.lower())
            if suggestion and suggestion not in suggestions:
                suggestions.append(suggestion)

        # Include score-driven actions when the failure type is missing or
        # non-standard, and ensure a useful minimum for a non-empty batch.
        score_actions = [
            "Audit retrieved context quality and add missing evidence to the corpus",
            "Add targeted golden and regression cases for the observed failures",
            "Calibrate evaluation thresholds against human-reviewed examples",
        ]
        for suggestion in score_actions:
